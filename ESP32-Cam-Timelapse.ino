#include "Arduino.h"
// SD Card Files
#include "esp_vfs_fat.h"       // SdFat File Support for SD cards >4GB
...
#define PICTURE_DELAY  3             // Delay for taking pictures (in seconds) after a previous picture has been taken
#define STARTUP_DELAY  0            // Optional Time ESP32 will sleep on first boot in seconds. Max delay is 584942 years before it overflows, so it wont be an issue soon
...
void setup() {
  ...
  // Camera Quality assuming 3.8GB of usable space (Supports up to 4TB)
  ...
  if (psramFound()) {
    Serial.println("Found PSRAM, choosing higher quality");
    config.frame_size = FRAMESIZE_UXGA;
    config.jpeg_quality = 10;
    config.fb_count = 2;
  } else {
    Serial.println("PSRAM not found, choosing lower quality");
  ...
  if (ret != ESP_OK) {
    if (ret == ESP_FAIL) {
      Serial.println("Failed to mount filesystem. If you want the eMMC to be formatted, set the EXAMPLE_FORMAT_IF_MOUNT_FAILED menuconfig option.");
    } else {
      Serial.print("Failed to initialize the eMMC (%s). Make sure eMMC lines have pull-up resistors in place.");
  ...
  char imgpath[64];
  snprintf(imgpath, 64, "/sdcard/pic_%lu.jpg", ++bootCount);
  FILE *file = fopen(imgpath, "w");
  if(!file) Serial.println("Failed to open file");
  else fwrite(fb->buf, sizeof(fb->buf[0]), fb->len, file); 
  fflush(file);
  fclose(file);
